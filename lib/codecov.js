...
'                                ' + version);
  ...
  if (!query.commit) {
    console.log('Exit. Commit sha was unable to be detected.')
    console.log('Suggestion: Include commit sha manually via --commit=build-commitid')
    process.exit(args.options.fail);
  }
  ...
  if (!query.token && !query.slug) {
    console.log('Exit. Missing upload details. Token or slug is required.')
    console.log('Suggestion: Include your upload token via --token=uuid-upload-token')
    process.exit(args.options.fail);
  }
  ...
  if (files) {
    ...
    if (args.options.dump) {
      ...
      sendToCodecovV3(codecov_endpoint, query, upload,
                      on_success || function(){},
                      on_failure || function(){process.exit(args.options.fail);});
    }
  }
