...
'                                ' + version);
  ...
  if (query.yaml) {
    try {
      var parsedYaml = yaml.safeLoad(fs.readFileSync(path.resolve(process.cwd(), query.yaml), 'utf8'));
      console.log(parsedYaml);
    } catch (e) {
      if (on_failure) {
        on_failure(e);
      } else {
        console.log('ERROR: Yaml parser failed with error: ' + e);
        return;
      }
    }
  }
  ...
  if (token) {
    query.token = token;
  } else if (parsedYaml.codecov.token) {
    query.token = parsedYaml.codecov.token;
  }
