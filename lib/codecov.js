...
'                                ' + version);
  ...
  // TODO: we should set other values here too based off yaml file
  // if a yaml file was provided, then parse it and look for the token (gh #23)
  if (query.yaml) {
    var yaml = YAML.load(path.resolve(process.cwd(), query.yaml));
    if (yaml.codecov && yaml.codecov.token)
      query.token = yaml.codecov.token;
    else
      throw new Error('yaml file detected but `codecov` block missing');
  }
